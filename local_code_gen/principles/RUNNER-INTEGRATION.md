...
│                                                                  │
│  Pipeline: local_code_gen/sprint_runner_qwen.dip                 │
...
## Fix: section-name alignment in `local_code_gen/sprint_runner_qwen.dip`
...
The dip validates clean post-fix (`tracker validate local_code_gen/sprint_runner_qwen.dip` → 19 pre-existing harmless warnings, 0 errors).
...
Workdir: `experiments/notebook_smoke_v4/`. Architect already produced specs (3-sprint Notebook API). Runner executed `local_code_gen/sprint_runner_qwen.dip` cold on those specs.
...
| `pipelines/local_code_gen/sprint_runner_qwen.dip` | Audit gate scans `## New files` + `## Modified files` (with legacy fallback); test-count uses `## Test contract` (with v1 fallback); Generate prompt hint mentions both v4 and v1 section names |

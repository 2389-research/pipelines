...
| [`RUNNER-INTEGRATION.md`](RUNNER-INTEGRATION.md) | Runner-side integration: section-name alignment between v4 architect output and the SR runner; 4-strategy SR-block matcher port from `lib/merge_sr.py` to the auditor; end-to-end Notebook run produces 34/34 passing tests for ~$2.30; defect-class observation (dependency errors vs logic errors) and proposed early-escalation heuristic. |

...
[ "$head" != "$base_commit" ] || { printf 'no task commit was created\n' >&2; exit 1; }
git merge-base --is-ancestor "$base_commit" "$head" || { printf 'task history no longer descends from the claimed base\n' >&2; exit 1; }
[ -s "$base/verification.txt" ] || { printf 'verification evidence is missing\n' >&2; exit 1; }
[ -s "$base/completion.md" ] || { printf 'completion summary is missing\n' >&2; exit 1; }
...
test_evidence=$(head -n 1 "$base/verification.txt")
completion=$(cat "$base/completion.md")
...
git merge-base --is-ancestor "$trunk_commit" "$head" || {
  printf 'trunk %s moved from %s to %s since the claim; rebase the task branch on it and rerun the reviews\n' \
    "$trunk" "$base_commit" "$trunk_commit" >&2
  exit 1
}

...
# writes an actionable line to $RUN_DIR/persist_plan_error.txt; the trap
      ...
      if [ "${rc}" -ne 0 ]; then
        [ -s "${RUN_DIR}/persist_plan_error.txt" ] \
          || printf "unexpected non-zero exit (rc=%s)\n" "${rc}" \
             > "${RUN_DIR}/persist_plan_error.txt" 2>/dev/null || true
...
response="${dip_artifact_dir}PlanMinimalPRs/response.md"

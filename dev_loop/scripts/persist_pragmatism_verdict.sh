#!/bin/sh
# persist_pragmatism_verdict.sh — capture SquadPragmatism's JSON verdict to disk.
# Emits: persisted-pragmatism | persist-failed (issue #48).
set -eu
      ...
      if [ "${rc}" -ne 0 ]; then
        [ -s "${RUN_DIR}/persist_pragmatism_error.txt" ] \
          || printf "unexpected non-zero exit (rc=%s)\n" "${rc}" \
             > "${RUN_DIR}/persist_pragmatism_error.txt" 2>/dev/null || true
...
if [ -z "${DIP_ARTIFACT_DIR:-}" ]; then
  ...
  printf 'unset' > "${RUN_DIR}/persist_pragmatism_fail_class.txt"
  exit 1
...
response="${dip_artifact_dir}SquadPragmatism/response.md"
target="${RUN_DIR}/verdict_pragmatism.json"

if [ ! -f "${response}" ]; then
  printf 'response missing at %s\n' "${response}" \
    > "${RUN_DIR}/persist_pragmatism_error.txt"
  printf 'response-missing' > "${RUN_DIR}/persist_pragmatism_fail_class.txt"
  exit 1
fi

if ! jq '.' < "${response}" > "${target}.tmp" 2> "${RUN_DIR}/persist_pragmatism_error.txt"; then
  printf 'jq-parse' > "${RUN_DIR}/persist_pragmatism_fail_class.txt"
  exit 1
fi

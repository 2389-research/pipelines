...
- 2026-09-18: plan written, no task started. The writing session had compacted 15 times; it retires after committing this plan. Doctor Biz chose option 1 of the review menu ("Fix criticals and importants as planned, then merge to main locally"), so execution and the local merge are authorized. Pushing `main` is not.
- Branch: `fix/kata-scripts-by-path` (forked from `main` at 1aeddf7; `main` has not moved). Commits so far: 686ee42, 09ff603, 0b23d91.
